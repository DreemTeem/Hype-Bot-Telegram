...
export class DotaMatches {
  ...
  private showCommandError(msg: any, command: string): void {
    this.HBot.sendMessage(
      msg.chat.id,
      `I'm sorry, I can't yet process the \\${command} command for /dota.`
    );
  }

  private showDotaMatches(msg: any): void {
    this.HBot.sendMessage(msg.chat.id, "Fetching Live Dota 2 matches...!");
    this.getLiveMatchData().then((matchString: string) => {
      this.HBot.sendMessage(msg.chat.id, matchString, {
        parse_mode: "HTML",
  ...
  private showDotaStreams(msg: any): void {
    ...
    this.getDotaLiveStreams().then(
      (streams: any[]) => {
        let messageText: string =
          "<b>Live Dota 2 Streams</b>\n==============================\n";
        ...
        for (let i: number = 0; i < 5; i++) {
          ...
          messageText +=
            ...
            "'>Watch @" +
            streams[i].channel.name +
            "</a>\n";
        }

        this.HBot.sendMessage(msg.chat.id, messageText, {
          parse_mode: "HTML",
          disable_web_page_preview: true,
        });
      },
      (error: any) => {
        this.HBot.sendMessage(msg.chat.id, error, {
          parse_mode: "HTML",
          disable_web_page_preview: true,
        });
      }
  ...
  private getLiveMatchData(): Promise<string> {
    return new Promise((resolve: any, reject: any) => {
      try {
        GosuAPI.fetchMatchUrls("dota2", null, (fetchError: any, urls: any) => {
          let matchesString =
            "<b>Current Live Dota 2 Matches</b>\n==============================\n";
          GosuAPI.parseMatches(urls, (parseError: any, matches: any) => {
            if (!parseError) {
  ...
  private getDotaLiveStreams(): Promise<any> {
    return new Promise((resolve: any, reject: any) => {
      const twitchURL: string =
        "https://api.twitch.tv/kraken/streams?game=Dota%202&client_id=xmrewln6tf1uwgvdwqbofprp8mqxqk";
      const request = require("request"); // TODO: Use fetch instead?s
      request.get(twitchURL, (error: any, response: any, body: any) => {
        if (response) {
          resolve(
            JSON.parse(response.body).streams.filter(
              (stream: any) => stream.channel.broadcaster_language === "en"
            )
          );
        } else if (error) {
          reject(error);
