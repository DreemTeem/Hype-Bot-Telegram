...
export class TextResponses {
  ...
  private setVolcelResponse(): void {
    this.HBot.onText(/^\/volcel/i, (msg: any, match: any): void => {
      ...
      this.HBot.sendPhoto(msg.chat.id, VOLCEL_POLICE_IMG, {
        caption: volcelResponse,
      });
    });
  }
  ...
  private setBasicHypeResponses(): void {
    this.HBot.onText(/h+y+p+e+/i, (msg: any, match: any): void => {
      this.HBot.sendMessage(
        msg.chat.id,
        hypeResponses[Math.floor(Math.random() * hypeResponses.length)]
      );
    });
  }
  ...
  private setBasicTextCommands(): void {
    ...
    //   this.HBot.sendMessage(
    //     msg.chat.id,
    //     "REALLL SOVIETTTT DAAAAAMAAAAAAAAAGGGGGEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE"
    //   );
    // });
  }
}
